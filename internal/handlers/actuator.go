...
func (h *ActuatorHandler) subscribeEvents() {
	token := h.mqtt.Subscribe("+/+/event", 1, func(client mqtt.Client, msg mqtt.Message) {
		...
		log.Printf("Evento recibido [%s]: %s", msg.Topic(), payload)

		var data struct {
			Relay int    `json:"relay"`
			State string `json:"state"`
		}
		if err := json.Unmarshal(msg.Payload(), &data); err != nil || data.Relay == 0 || data.State == "" {
			h.db.Exec("INSERT INTO actuator_events (actuator_id, state, source) VALUES (?, ?, 'mqtt')", 0, payload)
			return
		}
		...
		if normalized == "relay_on" || normalized == "turn_on" {
			normalized = "on"
		} else if normalized == "relay_off" || normalized == "turn_off" {
			normalized = "off"
		}

		h.db.Exec("INSERT INTO actuator_events (actuator_id, state, source) VALUES (?, ?, 'mqtt')", actuatorID, normalized)
		if normalized != currentState {
			h.db.Exec("UPDATE actuators SET state = ? WHERE id = ?", normalized, actuatorID)
		}
	})
	token.Wait()
	log.Println("Suscrito a +/+/event")
}
...
func (h *ActuatorHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		rows.Scan(&a.ID, &a.DeviceID, &a.Name, &a.Type, &a.RelayNum, &a.State, &a.CreatedAt)
...
func (h *ActuatorHandler) control(w http.ResponseWriter, r *http.Request, command string) {
	...
	err = h.db.QueryRow(
		"SELECT a.relay_num, a.device_id, a.type FROM actuators a JOIN devices d ON a.device_id = d.id WHERE a.id = ? AND d.user_id = ?",
		actuatorID, userID,
	).Scan(&relayNum, &deviceID, &actuatorType)
	if err != nil {
	...
	h.db.Exec(
		"INSERT INTO actuator_events (actuator_id, state, source) VALUES (?, ?, 'http')",
...
func (h *ActuatorHandler) Events(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.db.Query(`
		SELECT id, actuator_id, state, source, created_at
	...
	for rows.Next() {
		var e Event
		rows.Scan(&e.ID, &e.ActuatorID, &e.State, &e.Source, &e.CreatedAt)
